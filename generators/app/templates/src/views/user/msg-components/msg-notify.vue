<template>
  <!-- 单个消息提示弹框 start -->
  <el-dialog custom width="54%" :visible.sync="notifyDialog" append-to-body>
    <template slot="title">
      <label>消息</label>
    ...
    <idss-widget shadow="never" :main-style="{padding: '15px 0'}" v-loading="loading['notify']">
...
import { charCodeLength } from 'fusion-utils'
import BusFactory from '@bus'
import globalMixins from '@/mixins/globalMixins'
import service from './msg-notify-service.js'
import { MESSAGE_CENTER } from '@/custom/dictionaries/websocket.js'
...
export default {
  ...
  methods: {
    ...
    msgTemplate (data) {
      ...
      return h('div',
        { attrs: { class: 'idss-notify--single' } },
        [
          h('h1', `标题：【${data.title}】`),
          h('p', `${data.text}`)
        ]
    ...
    async startSoket () {
      ...
      this.bus.$on(`${MESSAGE_CENTER.WEBSOCKET_UNREAD_MSG_SINGLE}`, data => {
        let instanceUnreadMsgSingle = this.$notify({
          ...
          duration: 20000,
          onClick: (notify) => { this.notifyCheck(instanceUnreadMsgSingle) },
          offset: 90
        })
      })
