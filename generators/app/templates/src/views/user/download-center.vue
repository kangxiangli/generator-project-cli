<template>
  <div v-loading="loading['global-container']">
            ...
            <el-row :gutter="10">
              <el-col :span="6">
                <el-form-item label="任务名称：" label-width="90px">
                  <el-input v-model.trim="formData.taskName" placeholder="请输入任务名称"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="8">
                <el-form-item label="时间：" label-width="60px">
                  <idss-time-select :time.sync="globalTime" style="width: 100%;"></idss-time-select>
                </el-form-item>
              </el-col>
              <el-col :span="10">
    ...
    <!-- 下载任务列表 start -->
    <el-row>
      <idss-widget :header-style="{padding: '10px 15px'}" :footer-style="{padding: '6px 0'}">
        <template slot="title-left">下载任务列表</template>
        <template slot="title-left">
          <el-button size="small" class="idss-margin--l-s" :disabled="!selectedData.length" type="danger" plain @click="remove(selectedData)">
        ...
        <el-table
          ...
          <el-table-column
            ...
            align="center"
            width="160"></el-table-column>
          <el-table-column prop="taskStatus" label="任务状态" align="center" width="140px">
            <template slot-scope="scope">{{changeStr(scope.row.taskStatus)}}</template>
          </el-table-column>
          <el-table-column label="操作" align="center" width="130px">
            <template slot-scope="scope">
              <el-button
                plain
                size="mini"
                :disabled="(scope.row.taskStatus !== '2' && scope.row.taskStatus !== '4') || scope.row.flag"
        ...
        <template slot="footer">
          <idss-pagination
            ...
            @size-change="tableListChange"></idss-pagination>
        </template>
      </idss-widget>
    </el-row>
    <!-- 下载任务列表 end -->
...
export default {
  ...
  methods: {
    ...
    changeStr (param) {
      ...
      let strMap = new Map([['0', '全部'], ['1', '进行中'], ['2', '已完成'], ['3', '失败'], ['4', '已完成']])
    ...
    async remove (selectedData) {
      ...
      this.confirmBox({ message: '要删除选中的数据吗？' }).then(async () => {
        ...
        this.tableChange({ type: 'submit' }, this.tableListData, this.getTableListData)
      })
    },
    // 下载
    download (params) {
      ...
      params.flag = true
      download(fileName)
      this.tableChange({ type: 'refresh' }, this.tableListData, this.getTableListData)
    },
    // 初始化方法
