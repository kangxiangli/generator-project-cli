...
export default {
  ...
  mutations: {
    ADD_VISITED_VIEWS: (state, view) => {
      ...
      // 是否已存在
      if (state.visitedViews.some(v => v._r === view.query._r && v.name === view.name)) return
    ...
    UPDATE_VISITED_VIEWS: (state, view) => {
      ...
      const visiteVal = state.visitedViews.find(val => val._r === view.query._r && val.name === view.name)
      if (visiteVal) {
      ...
      const catchVal = state.cachedViews.find(val => val._r === view.query._r && val.name === view.name)
