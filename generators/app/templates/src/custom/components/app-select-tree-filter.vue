    ...
    <el-popover
      ...
      <el-input
        ...
        <i
          ...
          :class="visible === true?'el-icon-arrow-up':'el-icon-arrow-down'"></i>
      </el-input>
      <div ref="refTree" class="s-treeData el-select-dropdown">
        <el-input size="mini" style="margin-bottom: 10px;" placeholder="输入名称筛选" v-model="filterVal"></el-input>
        <el-scrollbar
          tag="div"
  ...
  methods: {
    setPopoverWidth () {
      if (!this.popoverWidth) {
        let width = this.$el.querySelector('.idss-tree-select-input').clientWidth
    ...
    changeSelected (val) {
      ...
      this.$nextTick(_ => {
        if (!this.multiple) {
          let currentNode = this.$refs.treeHolder ? this.$refs.treeHolder.getNode(val) : null
          if (currentNode === null) {
            this.checkedNodes = ''
          } else {
          ...
          this.checkedNodes = this.$refs.treeHolder ? this.$refs.treeHolder.getCheckedNodes() : ''
        }
    ...
    updateChooseVal (nodes) {
      if (Array.isArray(nodes)) {
        let n = nodes.filter(item => item[this.defaultProps.id] !== undefined)
        if (this.multiple) {
          this.chooseVal = n.map(item => { return item[this.defaultProps.label] }).join(',')
        } else {
    ...
    filterNode (value, data) { // 模糊查询
      if (!value) return true
      let reg = new RegExp('^[a-zA-Z]+$')
      reg.test(value) && (value = value.toLocaleLowerCase()) // 匹配是否是全英文进行转换，数据对象中的英文支持小写
      if (this.relatedFields) { // 为了处理复合查询
        return (data[this.defaultProps.label].indexOf(value) !== -1 || data[this.relatedFields].indexOf(value) !== -1)
      } else {
    ...
    checkChange (node, checkedObj, halfCheckedNodes, halfCheckedKeys) {
      if (this.multiple) {
        ...
        let ids = this.$refs.treeHolder.getCheckedKeys().filter(item => item !== undefined)
        let fIds = ''
        if (ids.length !== 0) {
          fIds = ids.join(',')
        }
