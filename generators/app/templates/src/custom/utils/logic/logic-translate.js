...
export function getShowText (type, data) {
  ...
  switch (type) {
    case 'source':
      ...
      // 事件数据和自选数据类型区分
      const langType = sourceType === 'event' ? `source_${sourceType}` : `source_${sourceType}_${timeType}`
      return translate('logic', langType, data)
    // 展示数据类型的处理
    case 'exhibit':
      return translate('logic', type, [logicTypesConfig.exhibit.types[data.value]])
    case 'sort':
      // 处理排序条件
      const { sortStat } = data
    ...
    case 'variable':
      // 处理标记变量
      value = data.value
      return translate('logic', `variable`, value.filter(item => item.name).reduce((list, item, index) => {
...
export function translate (translateType = 'logic', type, ...args) {
  ...
  if (translateType === 'logic') {
    // souce数据源和其他动作区分
    const $lang = args[0].type === 'source' ? logicTypesConfig.source[type] : logicTypesConfig[type]
