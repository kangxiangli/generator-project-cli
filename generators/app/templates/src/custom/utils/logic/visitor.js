...
class Vistor extends RouteListener {
  ...
  enterDataSource (sourceTree) {
    ...
    if (children && children.length) {
      const datasource = this._setBeansDescToMap(children, 'Literal', 'beanValue', beans)
      // 判断有DataSource数据处理(只有自选数据和事件数据可以使用)
      if (Object.keys(datasource.fieldMap).length && ['origin', 'event'].indexOf(datasource.fieldMap.sourceType) !== -1) {
        // 父级等于root
  ...
  enterAggregate (sourceTree) {
    ...
    const children = groupValue && groupValue.array().getText() ? groupValue.array().String() : []
    ...
    const res = {
      ...
      tableList: this.fieldList.filter(item => groupbyList.indexOf(item.field) !== -1).concat(countField),
      variableList: this.variableList,
      parentVariableList: this.variableList,
      value: groupbyList
    }
  ...
  enterSort (sourceTree) {
    ...
    field = field && field.getText().indexOf($G4_CLASS_EMPTY_MSG) === -1
      ? this._resetCharts(field.getText()) : undefined
    sortStat = sortStat && sortStat.getText().indexOf($G4_CLASS_EMPTY_MSG) === -1
      ? this._resetCharts(sortStat.getText()) : undefined
    const res = {
      type: 'sort',
      pId: this.lastParent.key,
      tableList: this.lastParent.tableList,
  ...
  enterVariable (sourceTree) {
    ...
    const parserList = []
    // 变量类型处理正则匹配
    const typeReg = new RegExp(`^(${logicTypesConfig.variable.variaTypeEum.join('|')})(\\d+)$`)
    bean.forEach(source => {
      const children = source.beanLiteral()
      let variable = {}
      const variableMap = this._setBeansDescToMap(children, 'Literal', 'beanValue', source)
  ...
  enterAnalysis (sourceTree) {
    ...
    if (!datasource || !datasource.getText().startsWith('source')) {
      this.currentAnalysisIsAble = false
      // 添加空节点（source数据源添加节点）
      sourceTree.stop && this._entryEmptyNode(sourceTree, sourceTree.stop.stop - 2, '0')
    } else {
      this.currentAnalysisIsAble = true
    }
  }
   ...
   */
  _entryEmptyNode (sourceTree, startIndex = sourceTree.stop.stop, pId = this.lastParent.key) {
    // 获取当前父节点最后一个节点位置作为截取位置
    let endIndex = sourceTree.constructor === RouteParser.AnalysisContext ? sourceTree.stop.stop : sourceTree.parentCtx.stop.stop
    ...
    if (str.trim() === '|]') {
      const res = {
        pId: pId,
        addDisable: true, // 对展示按钮控制，展示操作，添加按钮隐藏
        key: pId === '0' ? `${++this.id}` : `${pId}_${this.analysisList.filter(item => item.pId === pId).length + 1}`,
        // 空节点默认添加节点错误
        error: {
          [EMPTY_ERROR]: true
        }
   ...
   */
  _setBeansDescToMap (treeList, keyList = 'name', valueNameList = 'value', parentCtx) {
    // 获取某个属性值
    ...
    const fieldMap = treeList.reduce((map, tree, index) => {
  ...
  _resetCharts (str, strWarpper = '') {
    ...
    const isNumber = str.match(/^[+-]?([0-9]*\.?[0-9]+|[0-9]+\.?[0-9]*)([eE][+-]?[0-9]+)?$/)
    ...
    if (isArray) {
      ...
      let res = []
      if (strArr) {
        res = strArr.split(',').map(item => this._resetCharts(item, strWarpper))
      }
      return res
  ...
  _pushList (data, ctx, parserErrorList) {
    // 过滤多余属性数据
    const keepData = Object.entries(data).filter(([key, value]) => $visitorFilterField.indexOf(key) === -1)
    const str = JSON.stringify(keepData)
    let id = this.existList[str]
    // 判断source数据且存在相同的情况时，判断如果上次数据源和这次不同，则不相同
    if (id && data.type === 'source') {
      const lastDs = Object.entries(this.lastDatasource).filter(([key, value]) => $visitorFilterField.indexOf(key) === -1)
    ...
    if (id) {
      ...
      data.key = data.pId === '0' ? `${++this.id}` : `${this.lastParent.key}_${this.analysisList.filter(item => item.pId === this.lastParent.key).length + 1}` // 不能使用时间戳，某些节点会在相同时间创建
      this.existList[str] = data.key
      this.analysisList.push(data)
      // 对该数据进行校验
      this.validate.valid(data, this.lastParent, ctx, parserErrorList, this.excuteErrorList)
    }
    // 记录父级数据
