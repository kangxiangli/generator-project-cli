...
class LogicErrorListener extends ConsoleErrorListener {
  ...
  syntaxError (recognizer, offendingSymbol, line, column, msg, e) {
    ...
    // 标记错误的文本
    const markText = offendingSymbol ? this.getTokenErrorDisplay(offendingSymbol) : ''
    ...
    if (parent && !parent.nativeError && logicTypes.indexOf(parent.constructor) > -1) {
   ...
   */
  _getCnText (text) {
    return text.replace(/(^|\b)(token recognition error|no viable alternative at input|mismatched|input|expecting|Identifier|missing|at|extraneous|rule)($|\b)/g, function ($0, $1, $2) {
      switch ($2) {
