...
export function getMode (filedList = []) {
  ...
  // 高亮配置处理
  ace.define('ace/mode/logic_highlight_rules', [
    'require',
    'exports',
    'module',
    'ace/lib/oop',
  ...
  ace.define('ace/mode/logic', [
    ...
    const MyHighlightRules = require('./logic_highlight_rules').MyHighlightRules
  ...
  langTools.addCompleter({
    getCompletions (editor, session, pos, prefix, callback) {
      ...
      // 动作类型添加
      autoList.push(...filterType.map(item => ({
        name: item,
        value: item,
        score: 1,
        meta: '动作'
